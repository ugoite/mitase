---
name: mitase-maintainer
description: Maintain a mitase-driven repository by updating the layered spec, preserving adjacent links, running active validation/workbench checks, and refreshing generated artifacts when the spec changes.
---
...
`mitase` for specification-driven development.
...
- Update implementation and test traceability instead of leaving spec edits as
  disconnected YAML.
- Run the repository checks that keep `mitase` trustworthy.
...
1. Inspect the current layered model before editing.
   - Run `mitase validate workspace .` or `mitase workbench project --workspace . --format json`
     to see current validation state and active projection output.
   - Read the relevant files under `docs/mitase/`.
...
3. Keep validation rules in mind.
   - `mitase validate workspace .` should stay green unless you are intentionally working
...
4. Refresh generated artifacts when relevant.
   - Regenerate docs derived from `docs/mitase/` when spec files change.
