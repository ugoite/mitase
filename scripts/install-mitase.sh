...
DEFAULT_VERSION_SELECTOR="v0.1.1"
tmp_dir=""
...
resolve_package_host() {
  printf '%s\n' "${MITASE_PACKAGE_HOST:-$DEFAULT_PACKAGE_HOST}"
}

resolve_package_scheme() {
  printf '%s\n' "${MITASE_PACKAGE_SCHEME:-$DEFAULT_PACKAGE_SCHEME}"
}

resolve_package_repository() {
  local repository="$1"

  if [[ -n "${MITASE_PACKAGE_REPOSITORY:-}" ]]; then
    printf '%s\n' "$MITASE_PACKAGE_REPOSITORY"
    return 0
  fi
