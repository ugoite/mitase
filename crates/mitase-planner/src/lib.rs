...
use mitase_diagnostics::Diagnostic;
use mitase_project_model::ValidationPreset;
...
pub fn validate_work_origin(index: &SpecIndex, origin: &WorkOrigin) -> Result<()> {
    ...
    if criterion.kind != mitase_spec_model::LocalAnchorKind::Criterion {
...
fn suggestion_digest(
    ...
    let authority = serde_json::json!({
        "schema": "mitase/work-target-suggestion-authority/v1",
        "criterion": criterion,
    ...
    hash.update(mitase_work_model::canonical_json_bytes(authority));
...
fn suggestion_budget_exceeds(
    ...
    limits: &mitase_project_model::SliceLimits,
) -> bool {
...
fn binding_criteria(binding: &ArtifactBinding) -> Vec<SpecAnchor> {
    binding
        ...
        .filter_map(|claim| match claim {
            ...
            | mitase_spec_model::TargetClaim::Verifies { criterion, .. } => Some(criterion.clone()),
            mitase_spec_model::TargetClaim::Documents { anchor }
            | mitase_spec_model::TargetClaim::Evidences { anchor } => Some(anchor.clone()),
            mitase_spec_model::TargetClaim::Enforces { rule } => Some(rule.clone()),
            mitase_spec_model::TargetClaim::GeneratedFrom { .. }
            | mitase_spec_model::TargetClaim::Exposes { .. } => None,
        })
        .collect()
}
        ...
        WorkOperation::Document => index
            ...
            .flat_map(|(anchor, binding)| {
                binding
                    ...
                    .iter()
                    .filter(move |target| {
                        binding.role == mitase_spec_model::BindingRole::Documentation
                            && target.claims.iter().any(|claim| {
                                matches!(claim, mitase_spec_model::TargetClaim::Documents { anchor: actual } if actual == criterion)
                            })
                    })
...
fn rebuild_split_slice(
    ...
    let blockers = original
        .blockers
        .iter()
        .filter(|diagnostic| diagnostic.rule_id != "MITASE-WORK-003")
        .cloned()
        .collect::<Vec<_>>();
                    ...
                    if matches!(target.lifecycle, TargetLifecycle::EnsurePresent)
                        && let Some(container) = resolve_target_in_workspace(
                            ...
                            &mitase_spec_model::ArtifactTarget {
                                ...
                                selector: mitase_spec_model::Selector::Marker {
                                    value: "crate".into(),
                                },
...
fn digest_json<T: Serialize>(value: &T) -> String {
    let bytes = mitase_work_model::canonical_json_bytes(
        serde_json::to_value(value).expect("serialize canonical work digest input"),
    );
    let mut hash = Sha256::new();
    hash.update(b"mitase/work-origin-closure-digest/v1\0");
    hash.update(bytes);
...
mod tests {
    ...
    fn implementation_add_does_not_pull_unapproved_missing_verification_target() {
        ...
        assert!(
            plan.diagnostics
                .iter()
                .any(|diagnostic| diagnostic.rule_id == "MITASE-WORK-001")
        );
    }
    ...
    fn explicit_add_rejects_a_target_declared_absent() {
        ...
        assert_eq!(plan.status, PlanStatus::Blocked);
        assert!(plan.diagnostics.iter().any(|diagnostic| {
            diagnostic.rule_id == "MITASE-WORK-001"
                && diagnostic
                    .message
                    .contains("outside the exact editable origin closure")
        }));
        ...
        for target_id in ["operation-missing", "pointer-missing"] {
            ...
            assert!(plan.slices.iter().any(|slice| {
                slice
                    .blockers
                    .iter()
                    .any(|diagnostic| diagnostic.rule_id == "MITASE-TARGET-004")
            }));
        }
    }
    ...
    fn oversized_mixed_transition_closure_is_not_split_into_ready_slices() {
        ...
        fs::write(
            ...
            fs::read_to_string(tempdir.path().join("mitase.yaml"))
                .expect("config")
        ...
        assert!(
            plan.slices[0]
                .blockers
                .iter()
                .any(|diagnostic| diagnostic.rule_id == "MITASE-WORK-003")
        );
    }
    ...
    fn context_pack_distinguishes_target_and_support_entries_for_missing_targets() {
        ...
        assert!(
            plan.slices
                ...
                .any(|diagnostic| diagnostic.rule_id == "MITASE-WORK-015")
        );
    ...
    fn changed_unit_rejects_inactive_and_generated_artifacts() {
        ...
        assert!(
            plan.slices
                .iter()
                .flat_map(|slice| &slice.blockers)
                .any(|diagnostic| diagnostic.rule_id == "MITASE-WORK-013")
        );
    }
    ...
    fn inactive_build_profile_target_never_enters_executable_scope() {
        ...
        assert!(index.artifact_units.iter().any(|unit| {
            unit.identity.contains("handler")
                && matches!(
                    unit.reachability,
                    mitase_inventory::ArtifactReachability::Conditional { .. }
                )
        }));
        let request = WorkRequest {
            schema: WORK_REQUEST_SCHEMA.into(),
        ...
        assert!(
            plan.slices[0]
                .blockers
                .iter()
                .any(|diagnostic| diagnostic.rule_id == "MITASE-TARGET-002")
        );
