...
impl DesktopLaunchConfig {
    pub fn from_workspace(workspace_root: impl Into<PathBuf>) -> Self {
        ...
        Self {
            spec_root: workspace_root.join("docs/syu"),
            workspace_root,
...
pub fn run_with_config(config: DesktopLaunchConfig) -> Result<()> {
    println!(
        "Syu Workbench desktop shell is available with `--features tauri-runtime`; shared server URL: {}",
        config.local_url()
    );
    Ok(())
}
...
pub fn run_with_config(config: DesktopLaunchConfig) -> Result<()> {
    ...
    tauri::Builder::default()
        ...
        .context("failed to run Syu Workbench desktop shell")
}
...
mod tests {
    ...
    fn desktop_shell_points_at_the_shared_workbench_server() {
        ...
        assert_eq!(config.spec_root, PathBuf::from("/tmp/project/docs/syu"));
