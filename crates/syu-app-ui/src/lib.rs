...
impl<'a> WorkbenchView<'a> {
    ...
    fn render_settings(&self) -> String {
        ...
        format!(
            "<section data-page=settings data-config='{}' hidden><div class=eyebrow>UTILITY</div><h1>Settings</h1><div class=toolbar><code>syu/config/v1 · syu.yaml</code><button data-action=view-config>View YAML</button><button data-action=preview-config>Preview diff</button><button class=primary data-action=apply-config>Validate &amp; Apply</button></div><div class=settings><nav><button class=active data-settings-tab=workspace>Workspace</button><button data-settings-tab=profiles>Profiles &amp; Facets</button><button data-settings-tab=validation>Validation</button><button data-settings-tab=planning>Work Planning</button><button data-settings-tab=adapters>Adapters</button></nav><article><div data-settings-panel=workspace><h2>Workspace</h2><p>Repository roots and exclusions used by the canonical workspace loader.</p><label>Specification roots<input data-config-field=spec-roots value=\"{}\"></label><label>Artifact roots<input data-config-field=artifact-roots value=\"{}\"></label></div><div data-settings-panel=profiles hidden><h2>Profiles &amp; Facets</h2><label>Active profiles<input data-config-field=profiles value=\"{}\"></label></div><div data-settings-panel=validation hidden><h2>Validation</h2><label>Preset<select data-config-field=preset><option value=standard {}>Standard</option><option value=strict {}>Strict</option><option value=agent-ready {}>Agent ready</option></select></label><label class=check><input type=checkbox data-config-field=deny-warnings {}> Deny warnings</label><label class=check><input type=checkbox data-config-field=require-owned {}> Require owned changes</label></div><div data-settings-panel=planning hidden><h2>Work Planning</h2><div class=form-grid><label>Editable files<input type=number min=1 data-config-field=max-editable-files value=\"{}\"></label><label>Editable symbols<input type=number min=1 data-config-field=max-editable-symbols value=\"{}\"></label><label>Verification targets<input type=number min=1 data-config-field=max-verification value=\"{}\"></label><label>Readonly targets<input type=number min=1 data-config-field=max-readonly value=\"{}\"></label><label>Total bytes<input type=number min=1 data-config-field=max-bytes value=\"{}\"></label></div></div><div data-settings-panel=adapters hidden><h2>Adapters</h2><label>Enabled adapters<input data-config-field=adapters value=\"{}\"></label></div><textarea data-config-editor hidden></textarea><pre data-preview-output hidden></pre></article></div></section>",
...
@media(max-width:760px){.sidebar{width:64px}.toolbar{display:grid;grid-template-columns:minmax(0,1fr) 1fr}.toolbar select,.toolbar input,.toolbar code{min-width:0;grid-column:1/-1}.toolbar button{min-width:0}.topbar{height:68px}.palette{width:calc(100vw - 120px)}.filters{display:none}.item-list article{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;min-width:0;width:100%}.item-list article button{min-width:0;padding-inline:10px}}
</style></head>"#;
...
const safe=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),diagPhase=id=>{id=String(id).toUpperCase();return id.includes('WORK')||id.includes('PLAN')||id.includes('SLICE')?'work':id.includes('TARGET')||id.includes('BIND')||id.includes('CONTRACT')?'target':id.includes('CHANGE')||id.includes('RANGE')||id.includes('OWN')?'change':id.includes('SPEC')||id.includes('ANCHOR')||id.includes('GRAPH')?'spec':'config'},renderDiagnostics=v=>{const host=q('[data-page=diagnostics] .diagnostics');host.innerHTML=v.diagnostics.length?v.diagnostics.map(d=>`<article class=diagnostic data-status="${safe(d.severity)}" data-phase="${diagPhase(d.rule_id)}"><header><span aria-label="${safe(d.severity)}">●</span><div><b>${safe(d.rule_id)}</b><h2>${safe(d.message)}</h2></div></header><code>${safe(d.primary.path)}:${safe(d.primary.line??'-')}</code><p>${safe(d.help??'')}</p></article>`).join(''):'<div class=empty><span class=ok aria-label="success">●</span><h2>No diagnostics</h2><p>The selected validation context passed.</p></div>'};
...
const params=new URL(bootUrl).searchParams;for(const [name,selector] of [['tab','data-work-tab'],['view','data-scope-tab'],['kind','data-item-kind'],['phase','data-phase'],['section','data-settings-tab']]){const value=params.get(name),button=value&&q(`[${selector}="${value}"]`);button?.click()}history.replaceState({},'',bootUrl);
