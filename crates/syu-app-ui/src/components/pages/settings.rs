...
pub fn SettingsPage(
    ...
    rsx! {
        ...
        nav { class: "mb-3 flex flex-wrap gap-1 border-b border-slate-200", "aria-label": "Settings sections", for item in TABS { a { class: tab_class(item == tab), href: page_href(WorkbenchPage::Settings, ui.locale, Some(item), None, None), SyuIcon { name: item.icon(), size: 16 } "{copy.section_title(item)}" } } }
        section { id: "workspace-configuration", class: if focused { "rounded-lg border-2 border-red-500 bg-slate-50 p-4" } else { "rounded-lg border border-slate-200 bg-slate-50 p-4" }, "data-command-target": "workspace-configuration", tabindex: "-1",
            match tab {
                PageSection::General => rsx! { SyuYamlSettings { locale: ui.locale, spec_root, bind, port, strict_review, raw_yaml } },
                PageSection::App => rsx! { AppSettings { locale: ui.locale, root } },
...
fn SyuYamlSettings(
    ...
    rsx! {
        div { class: "flex flex-wrap items-center justify-between gap-3", div { p { class: "text-[10px] uppercase tracking-[0.2em] text-slate-400", "Utility page" } h2 { class: "mt-1 text-lg font-semibold", "syu.yaml" } } div { class: "flex items-center gap-2", StatusCircle { status: IndicatorStatus::Warning, label: if locale == Locale::Ja { "未適用の変更".to_string() } else { "unapplied changes".to_string() }, count: Some(1) } span { class: "rounded-full border bg-white px-2.5 py-1 text-xs", "source preserving" } } }
        div { class: "mt-4 grid gap-3 lg:grid-cols-2",
            form { class: "rounded-lg border border-slate-200 bg-white p-4", "data-settings-form": "true", h3 { class: "text-xs font-medium uppercase tracking-wide text-slate-500", if locale == Locale::Ja { "構造化設定" } else { "Structured settings" } } div { class: "mt-3 grid gap-3", SettingField { label: "Spec root".to_string(), name: "spec_root".to_string(), value: spec_root } SettingField { label: "Workbench bind".to_string(), name: "bind".to_string(), value: bind } div { class: "grid gap-3 sm:grid-cols-2", SettingField { label: "Port".to_string(), name: "port".to_string(), value: port.to_string() } SettingField { label: "Strict review".to_string(), name: "strict_review".to_string(), value: strict_review.to_string() } } label { class: "grid gap-1 text-[10px] uppercase tracking-[0.18em] text-slate-500", "Agent profile" input { class: "rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm normal-case tracking-normal text-slate-500", value: "local-codex (dry run first)", readonly: true } } } details { class: "mt-3", summary { class: "cursor-pointer text-xs", if locale == Locale::Ja { "YAML を直接編集" } else { "Edit YAML directly" } } textarea { class: "mt-2 min-h-40 w-full rounded-lg border border-slate-300 p-3 font-mono text-xs", name: "raw_yaml", readonly: true, "{raw_yaml}" } } }
...
#[component]
fn AppSettings(locale: Locale, root: String) -> Element {
    rsx! { h2 { class: "text-lg font-semibold", "App" } p { class: "mt-1 text-xs text-slate-500", "{root}" } div { class: "mt-4 grid gap-3 md:grid-cols-2", SettingSummary { label: if locale == Locale::Ja { "言語".to_string() } else { "Language".to_string() }, value: locale.slug().to_string() } SettingSummary { label: if locale == Locale::Ja { "テーマ".to_string() } else { "Theme".to_string() }, value: "system".to_string() } } }
}
#[component]
fn IntegrationSettings(locale: Locale) -> Element {
    rsx! { h2 { class: "text-lg font-semibold", "Integrations" } div { class: "mt-4 divide-y rounded-lg border border-slate-200 bg-white", for name in ["GitHub", "External agents"] { label { class: "flex items-center justify-between px-4 py-3 text-sm text-slate-400", "data-planned-setting": "true", span { "{name}" } span { class: "flex items-center gap-2", span { class: "rounded bg-slate-100 px-2 py-0.5 text-[10px]", if locale == Locale::Ja { "予定" } else { "Planned" } } input { type: "checkbox", disabled: true } } } } } }
}
#[component]
