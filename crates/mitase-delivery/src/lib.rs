...
pub const STORE_SCHEMA: &str = "mitase/completion/v1";
...
pub const MUTATION_JOURNAL_SCHEMA: &str = "mitase/workspace-mutation-journal/v1";
...
impl DeliveryStore {
    pub fn for_workspace(workspace_root: &Path) -> Result<Self> {
        let output = Command::new("git")
            ...
            .output()
            .context("resolve git mitase path")?;
        if !output.status.success() {
            bail!("git rev-parse --git-path mitase failed");
        }
    ...
    fn execute_and_append_attempt_for_agent_while_locked(
        ...
        let (verification, receipt, mut report) = mitase_validation::execute_verification_attempt(
            workspace,
            &index,
            plan,
            slice_id,
    ...
    pub fn finalization_preview_while_locked(
        ...
        if attempt.report.status != CompletionStatus::Complete || attempt.receipt.is_none() {
            blockers.push(CompletionBlocker {
                code: "MITASE-FINALIZE-INCOMPLETE".into(),
        ...
        if let Some(receipt) = &attempt.receipt {
            ...
            {
                blockers.push(CompletionBlocker {
                    code: "MITASE-FINALIZE-STALE-EVIDENCE".into(),
        ...
        let promoted_items = if blockers.is_empty() {
            ...
            for target in &slice.editable_targets {
                if index.item_status.get(&target.reference.binding.item)
                    ...
                    || !matches!(target.lifecycle, mitase_work_model::TargetLifecycle::Stable)
                {
        ...
        let mut preview = FinalizationPreview {
            schema: "mitase/finalization-preview/v1".into(),
    ...
    pub fn record_agent_patch_after_apply_while_locked(
        ...
        if patch.schema != mitase_work_model::AGENT_PATCH_SCHEMA
        ...
        let record = AgentPatchRecord {
            ...
            plan_digest: run.plan_digest.clone(),
            slice_id: run.slice_id.clone(),
            status: mitase_work_model::AgentPatchStatus::Accepted,
    ...
    pub fn record_rejected_agent_patch_while_locked(
        ...
        let record = AgentPatchRecord {
            ...
            plan_digest: run.plan_digest.clone(),
            slice_id: run.slice_id.clone(),
            status: mitase_work_model::AgentPatchStatus::Rejected,
            writes: patch.writes.clone(),
            changes: vec![],
            before_workspace_fingerprint: before_workspace_fingerprint.into(),
    ...
    pub fn digest<T: Serialize>(value: &T) -> Result<String> {
        Self::digest_with_domain("mitase/agent-event-digest/v1\0", value)
    }

    pub fn verification_digest<T: Serialize>(value: &T) -> Result<String> {
        Self::digest_with_domain(mitase_work_model::VERIFICATION_RECEIPT_DIGEST_DOMAIN, value)
    }

    pub fn finalization_digest<T: Serialize>(value: &T) -> Result<String> {
        Self::digest_with_domain(mitase_work_model::FINALIZATION_RECEIPT_DIGEST_DOMAIN, value)
    }
...
fn validate_completion_attempt_schema(attempt: &CompletionAttempt) -> Result<()> {
    ...
    if let Some(receipt) = &attempt.receipt {
        ...
        if attempt.verification.status != mitase_work_model::VerificationAttemptStatus::Complete
    ...
    } else if attempt.verification.status != mitase_work_model::VerificationAttemptStatus::Failed
...
fn validate_completion_attempt_against_plan(
    ...
    match (&attempt.verification.status, &attempt.receipt) {
        (mitase_work_model::VerificationAttemptStatus::Complete, Some(receipt)) => {
        ...
        (mitase_work_model::VerificationAttemptStatus::Failed, None) => {
        ...
        CompletionStatus::Complete => {
            if !attempt.report.blockers.is_empty()
                ...
                || !matches!(
                    ...
                    mitase_work_model::VerificationAttemptStatus::Complete
                )
            {
...
fn validate_agent_patch_writes(
    ...
    for write in &patch.writes {
        ...
        if planned.access != mitase_work_model::TargetAccessMode::Editable
            || !matches!(
                planned.transition,
        ...
        let current = index.target(target).and_then(|declared| {
            mitase_workspace::resolve_target_in_workspace(workspace, declared).ok()
        });
        match write {
...
fn agent_write_target(write: &AgentTargetWrite) -> &mitase_spec_model::BoundTargetRef {
    match write {
...
fn require_agent_transition(
    target: &mitase_spec_model::BoundTargetRef,
    planned: &mitase_work_model::PlannedTarget,
    transition: mitase_work_model::TargetTransition,
    requires_file: bool,
) -> Result<()> {
    if planned.transition != transition
        || (transition == mitase_work_model::TargetTransition::Add
            && planned.lifecycle != mitase_work_model::TargetLifecycle::EnsurePresent)
        || (transition == mitase_work_model::TargetTransition::Remove
            && planned.lifecycle != mitase_work_model::TargetLifecycle::EnsureAbsent)
        || (transition == mitase_work_model::TargetTransition::Modify
            && planned.lifecycle != mitase_work_model::TargetLifecycle::Stable)
...
fn agent_change(
    planned: &mitase_work_model::PlannedTarget,
    current: Option<&mitase_workspace::ResolvedTarget>,
) -> AgentTargetChange {
    AgentTargetChange {
...
fn validate_agent_event_references(
    ...
    match &event.event {
        ...
        AgentEventKind::PatchRecorded { patch } => {
            if patch.status == mitase_work_model::AgentPatchStatus::Accepted {
            ...
            if validate_current_context {
                ...
                let expected = if patch.status == mitase_work_model::AgentPatchStatus::Accepted {
...
fn validate_finalized_workspace(root: &Path) -> Result<SpecWorkspace> {
    ...
    let index = workspace.index()?;
    let result = mitase_validation::validate(&mitase_validation::ValidationContext {
        ...
        work_plan: None,
        selected_slice: None,
        plan_mode: mitase_validation::PlanValidationMode::PostState,
            ...
            slice
                ...
                .filter_map(|target| {
                    let lifecycle = match target.lifecycle {
                        ...
                        mitase_work_model::TargetLifecycle::Stable => return None,
                    };
                    ...
                    if ids.contains(&item.id) {
                        ...
                        for binding in &mut item.bindings {
                            ...
                            for target in &mut binding.targets {
                                let reference = mitase_spec_model::BoundTargetRef {
...
mod tests {
    ...
    fn fixture_plan(root: &Path, revision: &str) -> mitase_work_model::WorkPlan {
    ...
    fn store_boundary_is_repository_local_and_explicit() {
        ...
        assert!(store.root().display().to_string().contains("mitase"));
        store.ensure().unwrap();
    ...
    fn receipt_digest_domains_have_literal_vectors() {
        ...
        assert_eq!(
            ...
            "sha256:487357ce3a05632f19a4011c98914dfcce8a0eb638dfc89682bc95db8e1e77ba"
        );
        assert_eq!(
            DeliveryStore::finalization_digest(&value).unwrap(),
            "sha256:bfbbb5627203f326e1931db761a4d728f91f3691cb4d91c3e8bca142c9e01a2d"
        );
    }
    ...
    fn approvals_require_canonical_scope_and_are_idempotent() {
        ...
        blocked.plan.status = mitase_work_model::PlanStatus::Blocked;
    ...
    fn durable_artifacts_require_their_declared_v1_schemas() {
        ...
        invalid_approval.schema = "mitase/legacy-approval/v0".into();
        ...
        invalid_attempt.schema = "mitase/legacy-attempt/v0".into();
        ...
        let invalid_finalization = FinalizationReceipt {
            schema: "mitase/legacy-finalization/v0".into(),
        ...
        let invalid_event = AgentEvent {
            schema: "mitase/legacy-agent-event/v0".into(),
        ...
        let missing_attempt_event = AgentEvent {
            schema: mitase_work_model::AGENT_EVENT_SCHEMA.into(),
        ...
        let run_json = |approval_id: &str| {
            serde_json::json!({
                "schema": mitase_work_model::AGENT_RUN_SCHEMA,
                ...
                "created_at": "0",
                "context": {
                    "schema": "mitase/agent-context/v1",
                    "plan_digest": identity.plan_digest.clone(),
                    "slice_id": identity.slice_id.clone(),
                    "context": {
        ...
        let late_blocker = AgentEvent {
            ...
            event: AgentEventKind::BlockerRecorded {
                blocker: mitase_work_model::AgentBlocker {
                    code: "late".into(),
    ...
    fn finalization_preview_requires_complete_attempt() {
        ...
        assert!(
            preview
                .blockers
                .iter()
                .all(|blocker| blocker.code != "MITASE-FINALIZE-STALE-EVIDENCE")
        );

        let mut incomplete = attempt.clone();
        incomplete.attempt_id = "attempt-incomplete".into();
        incomplete.attempt_digest.clear();
        incomplete.verification.status = mitase_work_model::VerificationAttemptStatus::Failed;
        incomplete.verification.executions.clear();
        incomplete.verification.failure = Some(mitase_work_model::VerificationAttemptFailure {
            code: "MITASE-FIXTURE-INCOMPLETE".into(),
        ...
        assert!(
            preview
                .blockers
                .iter()
                .any(|blocker| blocker.code == "MITASE-FINALIZE-INCOMPLETE")
        );
    }
    ...
    fn complete_attempt_requires_exact_completion_report_evidence() {
        ...
        complete.report.checks = slice
            ...
            .iter()
            .cloned()
            .map(|check| mitase_work_model::CompletionCheckEvidence {
                check,
                passed: true,
        ...
        complete.report.demonstrated = slice
            ...
            .iter()
            .map(
                |acceptance| mitase_work_model::CompletionCriterionEvidence {
                    ...
                    verification_targets: slice
                        ...
                        .filter(|target| {
                            target.verification_claim.as_ref().is_some_and(|claim| {
                                claim.criterion == acceptance.anchor
                                    && executed_claims.contains(claim)
                            })
                        })
                        .map(|target| target.reference.clone())
                        .collect(),
