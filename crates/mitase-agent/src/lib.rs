...
use mitase_inventory::ArtifactUnit;
use mitase_spec_model::{BoundTargetRef, format_sha256};
use mitase_validation::canonical_plan_for_execution;
...
pub const AGENT_CONTEXT_SCHEMA: &str = mitase_work_model::AGENT_CONTEXT_SCHEMA;
...
pub fn start_run(
    ...
    let context = mitase_planner::export_context(&plan, slice_id, workspace, &index, &revision)?;
    ...
    match result {
        ...
        Err(error) => {
            let blocker = AgentBlocker {
                code: "MITASE-AGENT-PATCH-REJECTED".into(),
...
pub fn request_scope_expansion(
    ...
    let store = DeliveryStore::for_workspace(&workspace.root)?;
    let request = mitase_work_model::ScopeExpansionRequest {
...
fn apply_patch_inner(
    ...
    let journal_files = files
        .iter()
        .map(|(path, mutation)| mitase_delivery::MutationJournalFile {
...
fn approved_plan(workspace: &SpecWorkspace, run: &AgentRun) -> Result<mitase_work_model::WorkPlan> {
    let store = DeliveryStore::for_workspace(&workspace.root)?;
    let identity = ExecutionIdentity {
        plan_digest: run.plan_digest.clone(),
        slice_id: run.slice_id.clone(),
...
fn validate_post_patch(
    before: &mitase_workspace::SpecIndex,
    candidate: &SpecWorkspace,
    after: &mitase_workspace::SpecIndex,
    ...
    for write in &patch.writes {
        ...
        let after_resolved = after.target(target).and_then(|declared| {
            mitase_workspace::resolve_target_in_workspace(candidate, declared).ok()
        });
...
fn is_file_target(target: &mitase_work_model::PlannedTarget) -> bool {
...
fn ensure_target_absent(
    ...
    if mitase_workspace::resolve_target_in_workspace(workspace, declared).is_ok() {
        bail!("target {target} now exists; refresh the plan before writing");
    }
    Ok(())
}
...
fn ensure_current_snapshot(
    target: &BoundTargetRef,
    planned: &mitase_work_model::PlannedTarget,
    resolved: &mitase_workspace::ResolvedTarget,
) -> Result<()> {
    if resolved.content_hash != planned.content_hash
...
fn lifecycle_proof(
    ...
    let current = index.target(&planned.reference).and_then(|declared| {
