function initializePreferences() {
  ...
  let formatPreference = localStorage.getItem('mitase.formatLocale') || 'interface';
  let numberFormatter = new Intl.NumberFormat('en');
  let dateFormatter = new Intl.DateTimeFormat('en');
  let preferences = (() => { try { return JSON.parse(localStorage.getItem('mitase.applicationPreferences') || '{}'); } catch { return {}; } })();

  function lookup(key) {
    return window.MITASE_I18N[locale]?.[key];
  }
  ...
  function translate(language) {
    ...
    document.title = locale === 'ja' ? 'Mitase ワークベンチ' : 'Mitase Workbench';
    ...
    localStorage.setItem('mitase.locale', locale);
    document.dispatchEvent(new CustomEvent('mitase:locale', { detail: { locale } }));
  }
  ...
  function applyPreferences() {
    ...
    localStorage.setItem('mitase.applicationPreferences', JSON.stringify(preferences));
  }
  ...
  function applyTheme(preference) {
    ...
    localStorage.setItem('mitase.theme', themePreference);
  }
  ...
  window.MitasePreferences = { translate, theme: applyTheme, settingsLayer: setLayer, settingsPage: setPage, t: required, lookup, formatNumber: value => numberFormatter.format(value), formatDate: value => dateFormatter.format(value) };
  translate(params().get('lang') || localStorage.getItem('mitase.locale') || osLanguage);
  applyTheme(params().get('theme') || localStorage.getItem('mitase.theme') || 'system');
  ...
  if (formatSelect) {
    ...
    formatSelect.addEventListener('change', () => { formatPreference = formatSelect.value; localStorage.setItem('mitase.formatLocale', formatPreference); translate(locale); });
  }
