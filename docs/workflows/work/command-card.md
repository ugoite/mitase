...
| Finalize a complete attempt | `syu task finalize preview/apply --attempt <attempt-id> --plan-digest <digest> --slice-id <slice-id> --workspace .` | revalidate and atomically promote only the exact planned slice |
...
| Export one slice context pack | `syu work export-context --plan plan.yaml --plan-digest <digest> --slice-id <slice-id> --workspace .` | materialize one validated slice into a portable context pack |
...
syu validate plan . --plan plan.yaml --plan-digest <digest> --slice-id <slice-id>
...
```bash
syu work export-context --plan plan.yaml --plan-digest <digest> --slice-id <slice-id> --workspace .
```
