...
The checked-in VS Code extension under `editors/vscode/` keeps `mitase` close to the
editor:

- validation diagnostics flow into the Problems panel
- the **mitase Context** explorer view shows the active file's linked
...
- diagnostics come from `mitase validate workspace . --format json`
...
editor protocol later. The repository now also ships `mitase lsp` as an
...
## How this relates to `mitase lsp`

The repository now also ships `mitase lsp`, but the VS Code extension is still
...
- **Still reads checked-in YAML directly**: the tree view, spec-ID lookups, and
  ...
  the middle
- **LSP server exists as a shared editor foundation**: `mitase lsp` already gives
...
[LSP guide](./lsp.md). Use `mitase lsp --help` for the current CLI surface and
transport flags.
...
2. treat `mitase lsp` as the place to grow editor-agnostic capabilities that
...
- hover is the first capability that can already be shared through `mitase lsp`
...
If the `mitase` binary is not already on your `PATH`, open VS Code settings and set
`mitase.binaryPath` to the installed CLI.
...
`mitase: Trace active file` uses the current text selection as a symbol when the
selection looks like an identifier. Without a selection, it shows file-level
ownership for the active file.

## Settings

- `mitase.binaryPath`: CLI path used for diagnostics
