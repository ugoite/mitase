...
- **id**: REQ-WORKBENCH-001
  - **title**: Role-oriented, command-palette-first Workbench
  - **description**:
    - |
      The Workbench MUST keep the command palette as its keyboard-first action
      surface while exposing only four persistent role menus: Items, Work,
      Scope, and Diagnostics. A selected palette command MUST open in its
      appropriate role instead of leaving the user in a generic command result.
      Items MUST provide a collapsible spec tree, linked-item navigation,
      source-preserving item editing with mandatory diff review, reciprocal
      adjacent-layer link updates, and workspace initialization when specs do
      not exist. Diagnostics MUST expose distinct workspace checks and refresh
      all available checks together. Work and Scope MAY expose focused
      subviews without adding more persistent roles. The shared header MUST
      remain visible above these roles, and the shell SHOULD use centralized
      design tokens and reusable classes.
  - **priority**: medium
  - **status**: implemented
  - **linked_policies**:
    - POL-005
    ...
    - **rust**:
      ...
      - **file**: crates/syu-workbench-server/src/tests.rs
        - **symbols**:
          ...
          - item_edit_previews_and_applies_reciprocal_links_together
          - diagnostics_refresh_all_runs_unique_tools_and_skips_missing_goal
          - items_surface_opens_before_workspace_initialization
