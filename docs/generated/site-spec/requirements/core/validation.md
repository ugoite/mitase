...
requirements:
  ...
  - id: REQ-CORE-002
    ...
    description: |
      ...
      path uses canonical repository-relative form, and any `doc_contains`
      snippets are present in the symbol documentation when the declared
      language supports rich inspection. The checked-in trace mapping itself
      MUST remain enough to audit ownership without requiring inline
      requirement or feature IDs in implementation files by default.
      Repositories that want an extra repository-native ownership breadcrumb
      MUST be able to opt into either inline IDs in traced artifacts or
      checked-in sidecar ownership manifests. Validation MUST also reject
      duplicate trace mappings inside a single language list, support wildcard
