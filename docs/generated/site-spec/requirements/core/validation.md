...
- **id**: REQ-CORE-002
  ...
  - **description**:
    - |
      ...
      language supports rich inspection. Ruby now uses the same
      declaration-adjacent line-comment inspection and ownership inventory as
      the richer adapters when the symbol shape is stable. The checked-in trace
      mapping itself MUST remain enough to audit ownership without requiring
      inline requirement or feature IDs in implementation files by default.
      Repositories that want an extra repository-native ownership breadcrumb
      MUST be able to opt into either inline IDs in traced artifacts or
      checked-in sidecar ownership manifests. Validation MUST also reject
      ...
      public Ruby classes, modules, and methods, public Java types and
      members, public C# types and members, `pub` for Rust, exported for
      TypeScript/JavaScript) and every test symbol
      (`test_*` functions for Python, `Test*`, `Benchmark*`, `Fuzz*`, and
      `Example*` functions in Go `_test.go` files, Ruby `test*` methods in
      `test/`, `tests/`, and `spec/`, JUnit `@Test` methods or legacy `test*`
      methods for Java, xUnit/NUnit/MSTest-attributed methods for C#,
      `#[test]` for Rust, `test*`-prefixed functions for
    ...
    - **go**:
      - **file**: tests/fixtures/workspaces/passing/go/traceability_test.go
        - **symbols**:
          - TestReqTraceGo
    - **java**:
      - **file**: tests/fixtures/workspaces/passing/java/TraceabilityTest.java
        - **symbols**:
          - reqTraceJavaTest
    - **typescript**:
...
- **id**: REQ-CORE-003
  ...
  - **description**:
    - |
      ...
      structural edits. It MAY also normalize canonical trace file spellings,
      repair legacy `planned` status typos, remove exact duplicate graph links,
      ...
      obvious. It MAY normalize deterministic ownership metadata in sidecar
      manifests when one safe correction exists. If a later write fails, it
      MUST roll back earlier writes so the workspace is not left half-updated.
  - **priority**: high
  - **status**: implemented
  - **linked_policies**:
    - POL-003
